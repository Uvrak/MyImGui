...
{
    MemoryReadTracker::recordMemoryWrite(
        address,
        val
    );

    const HostPt tlb_addr =
        get_tlb_write(address);
...
static INLINE void mem_writew_inline(
    const LinearPt address,
    const uint16_t val
)
{
    const PhysPt physicalAddress =
...
static INLINE void mem_writed_inline(
    const LinearPt address,
    const uint32_t val
)
{
