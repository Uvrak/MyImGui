...
Bits CPU_Core_Normal_Run(void) {
    ...
    while(CPU_Cycles-- > 0) {
        ...
        const LinearPt memoryWriteTarget =
            MemoryReadTracker::memoryWriteWatchTarget();

        MemoryReadTracker::RegisterSnapshot
            registerSnapshot;

        registerSnapshot.ax = reg_ax;
        registerSnapshot.bx = reg_bx;
        registerSnapshot.cx = reg_cx;
        registerSnapshot.dx = reg_dx;

        registerSnapshot.si = reg_si;
        registerSnapshot.di = reg_di;
        registerSnapshot.bp = reg_bp;
        registerSnapshot.sp = reg_sp;
        ...
        if(MemoryReadTracker::transitionTrackingIsActive())
        {
            MemoryReadTracker::recordTransitionInstruction(
                core.instruction_start,
                SegValue(cs),
        ...
        if(memoryWriteTarget != 0)
        {
            MemoryReadTracker::setCurrentInstructionContext(
                core.instruction_start,
                SegValue(cs),
                static_cast<uint16_t>(
                    reg_eip
